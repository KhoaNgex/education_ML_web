...
def show_predict_page():
    ...
    st.write(":man-girl-boy: Is that student living with his/her parents or living apart?")
    ...
    st.write(":bookmark_tabs: How much time does that student spend on self-studying every week?")
    ...
    schoolsup_select = st.radio("Extra school educational support", yes_no_list)
    ...
    st.write(":game_die: Student estimates his/her free time after school (band score from 1 to 5):")
    ...
    st.write(":male-doctor: Student evaluates his/her current health status (band score from 1 to 5):")
    ...
    st.write(":memo: Enter the first period score (mid-semester grade) of that student:")
    g1_select = st.number_input("Mid-semester Score", min_value=0, max_value=20, value=0)

    st.write(":memo: Enter the second period score (assignment grade) of that student:")
    g2_select = st.number_input("Assignment Score", min_value=0, max_value=20, value=0)

    st.subheader(":one: Student Information You've just enter (after encoding): ")
    student_info={
    ...
    object_attribute = ['school', 'sex', 'address', 'family_size', 'parents_status', 'mother_job', 'father_job', 'reason', 'school_support', 'family_support', 'paid_classes', 'activities', 'desire_higher_edu', 'internet']
    ...
    for attribute in object_attribute:
        le_name = "le_" + attribute
        student_info_np_encode[:, col_list.get_loc(attribute)] = globals()[le_name].transform(student_info_np_encode[:, col_list.get_loc(attribute)])
