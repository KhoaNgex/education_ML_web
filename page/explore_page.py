...
def display_perc_dist():
    ...
    if option == 'none':
        ...
        ax1.pie(data, labels=data.index, autopct='%.0f%%', textprops={'size': 'smaller'}, colors=colors)
        ax1.axis("equal") 
        st.pyplot(fig1)
...
def display_score_hist(option):
    ...
    bin_num = st.slider('Adjusting the number of bins', 1, 30, 12, disabled=discrete_cus)
    # draw histogram
    fig = plt.figure()
    if discrete_cus:
        bin_num = 'auto'
...
def display_mean_comparison(option):
    ...
    if mean_option == "none":
        ...
        mean_data = df.groupby([mean_option])[option].mean().sort_values(ascending=True)
        st.table(mean_data)
        if line_chart:
